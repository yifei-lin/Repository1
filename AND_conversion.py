...
def pick_random_tuple(tuples_list):
    return random.choice(tuples_list)
...
def group_AND_gate_nodes(adj_matrix, grouped_nodes, ungrouped, already_grouped, recorder):
    n = len(adj_matrix[-1])
    grouped_set = set()
    count = 0
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            ...
            if common_targets:
                ...
                grouped_set.add(j)
                count += 1
                recorder.record(i, j, 'new_group_id')  # Record the operation
                break
    if count > 0:
...
def AND_Gate_Conversion(original_matrix, adj_matrix, grouped_nodes, ungrouped, already_grouped, node_mapping, recorder,final_order_grouped,final_node_dic):
    
    grouped_nodes, ungrouped = group_AND_gate_nodes(adj_matrix, grouped_nodes, ungrouped, already_grouped, recorder)
    ...
    if len(already_grouped) == 0:
        while check_grouped:
            selected_tuple = pick_random_tuple(check_grouped)
            ...
            for i in range(n):
                ...
                for j in range(n):
                    ...
                    if stored_matrix[i][j] > 0:
                        if i != j:
                            if stored_matrix[i][j] > 1 and i in flattened_grouped_nodes:
                        #for k in range(n):
                         #   if adj_matrix[i][k] > 1:
                        ...
                        # Sum the weights to the new matrix and subtract 1
                            ...
                            if stored_matrix[i][j] > 1 and i not in flattened_grouped_nodes:
    ...
    if len(already_grouped) == 0:
        node_mapping = {}
        for j in flattened_grouped_nodes:
            
            node_mapping[j] = new_index // 2  # Grouped nodes get the same new index
            new_index += 1
            
        for i in range(n):  
            if i not in flattened_grouped_nodes:
                node_mapping[i] = new_index - grouped_node_count
                new_index+=1
        print("node_map: " + str(node_mapping))
    ...
    '''
        for i in range(n):
            ...
            for j in range(n):
                ...
                if adj_matrix[i][j] > 1 and i in flattened_grouped_nodes:
                    # Sum the weights to the new matrix and subtract 1
                    new_adj_matrix[new_i][new_j] = max(new_adj_matrix[new_i][new_j], adj_matrix[i][j] - 1)
                if adj_matrix[i][j] > 1 and i not in flattened_grouped_nodes:
                    new_adj_matrix[new_i][new_j] = adj_matrix[i][j]
                if adj_matrix[i][j] == 1:
    ...
    if len(already_grouped) > 0:
        while check_grouped:
            ...
            final_node_dic.append(node_mapping)
            discrete_matrix_len =  n - 1
            new_adj_matrix = np.zeros((discrete_matrix_len, discrete_matrix_len))
                ...
                for j in range(n):
                    ...
                    if stored_matrix[neww_i][neww_j] > 0:
                        if new_i != new_j:
                            ...
                            if stored_matrix[neww_i][neww_j] > 1 and new_i in flattened_grouped_nodes:
                        #for k in range(n):
                         #   if adj_matrix[i][k] > 1:
                          #      check_list.append((i,k))
                        # Sum the weights to the new matrix and subtract 1
                                new_adj_matrix[new_i][new_j] = max(new_adj_matrix[new_i][new_j], stored_matrix[neww_i][neww_j] - 1)
                            if stored_matrix[neww_i][neww_j] > 1 and new_i not in flattened_grouped_nodes:
        ...
        # Grouped nodes get the same new index
        '''
        new_adj_matrix = np.zeros((new_n, new_n))
        ...
        for i in range(n):
            ...
            if new_i in already_grouped:
                for j in range(n):
                    ...
                    if adj_matrix[neww_i][neww_j] != 0:
                        new_adj_matrix[new_i][new_j] = adj_matrix[neww_i][neww_j]
            ...
            if new_i not in already_grouped:
                for j in range(n):
                    ...
                    if adj_matrix[neww_i][neww_j] > 1 and neww_i in flattened_grouped_nodes:
               
                        if new_i != new_j:
                            
                            new_adj_matrix[new_i][new_j] = max(new_adj_matrix[new_i][new_j], adj_matrix[neww_i][neww_j] - 1)
                    
                    if adj_matrix[neww_i][neww_j] > 1 and neww_i not in flattened_grouped_nodes:
                            new_adj_matrix[new_i][new_j] = adj_matrix[neww_i][neww_j]
                    if adj_matrix[neww_i][neww_j] == 1:
                        if new_i != new_j:
                            new_adj_matrix[new_i][new_j] = adj_matrix[neww_i][neww_j]
        '''
