...
class digraphPlot(tk.Canvas, tk.Frame):
    ...
    def interactive_network(self):
        ...
        degrees = dict(self._digraph_normal.degree())
        ...
        for desc, node_id in self._node_ID.items():
            degree = degrees[str(node_id)]
            norm_degree = (degree - min_degree) / (max_degree - min_degree) if max_degree > min_degree else 0
            rgba = cmap(norm_degree)  # Get RGBA value from color map
            hex_color = matplotlib.colors.rgb2hex(rgba)  # Convert RGBA to hex format
            net.add_node(str(node_id), label=str(node_id), title=desc, size=10,color=hex_color)
        net.from_nx(self._digraph_normal)
        for edge in net.edges:
            edge['arrows'] = 'to'
            edge['arrowStrikethrough'] = False
            edge_weight = self._digraph_normal.edges[edge['from'], edge['to']].get('weight', 1) 
            edge['width'] = 0.1
            edge['scaling'] = {'min': 1, 'max': 5}
            from_node = int(edge['from'])
            to_node = int(edge['to'])
            edge_weight = self._adjacency_matrix[from_node, to_node]
            edge['title'] = f"Weight: {edge_weight}"
            edge['label'] = str(int(edge_weight))
        print('Done')
...
### Robustness of network focusing on largest connected component
    ...
    def get_causal_consequence_subgraph(self):
        ...
        # Dictionary of node descriptions
    
    
        # Find all paths from source nodes to sink nodes
        all_paths = []
        paths_info = []  # To store tuple of (source, sink, path description)
        
        id_to_description = {value: key for key, value in self._node_ID.items()}
        for source in source_nodes:
            for sink in sink_nodes:
                paths = list(nx.all_simple_paths(self._digraph_normal, source=source, target=sink))
                for path in paths:
                    print(id_to_description)
                    # Convert node indices to descriptions
                    path_descriptions = [id_to_description[int(node)] for node in path]
                    path_str = ' -> '.join(path_descriptions)  # Convert path list to a string for easy reading
                    paths_info.append((id_to_description[int(source)],id_to_description[int(sink)], path_str))
        ...
        # Add headers
        worksheet.write(0, 0, 'Source Description')
        worksheet.write(0, 1, 'Sink Description')
        worksheet.write(0, 2, 'Path Description')
    
        # Write data to worksheet
        for row_num, (source_desc, sink_desc, path_desc) in enumerate(paths_info, start=1):
            worksheet.write(row_num, 0, source_desc)
            worksheet.write(row_num, 1, sink_desc)
            worksheet.write(row_num, 2, path_desc)
